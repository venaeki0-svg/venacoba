...
interface ClientsProps {
    ...
    // CRUD function props (passed from App.tsx)
...
const Clients: React.FC<ClientsProps> = ({ clients, setClients, projects, setProjects, addClient, updateClient, deleteClient, addProject, updateProject, packages, addOns, transactions, setTransactions, addTransaction, updateTransaction, userProfile, showNotification, initialAction, setInitialAction, cards, setCards, updateCard, pockets, setPockets, updatePocket, contracts, handleNavigation, clientFeedback, promoCodes, setPromoCodes, updatePromoCode, onSignInvoice, onSignTransaction }) => {
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (modalMode === 'add') {
                // Create or reuse client in Supabase
                ...
                if (createdProject.amountPaid > 0) {
                    ...
                    const pocketToUpdate = pockets.find(p => p.id === 'POC005');
                    if (pocketToUpdate) {
                        await updatePocket(pocketToUpdate.id, { amount: pocketToUpdate.amount + createdProject.amountPaid });
                    }
                    if (formData.dpDestinationCardId) {
                        const targetCard = cards.find(c => c.id === formData.dpDestinationCardId);
                        if (targetCard) {
                            await updateCard(targetCard.id, { balance: targetCard.balance + createdProject.amountPaid });
                        }
                    }
                }
                if (promoCode) {
